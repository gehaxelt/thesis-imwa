...
/**
 * Calculation which implements Adler's EditOnly contribution measure.
 *
 * @see <a href="https://users.soe.ucsc.edu/~luca/papers/08/wikisym08-users.pdf">the paper</a>
 * Created by gehaxelt on 11.01.17.
 */
public class EditOnlyCalculation extends ACalculation<IContributor, Double, IRevision, ArgumentsBundle> {
    ...
    public void preProcess() {
        // Get the Revision DataSet and calculate the EditOnly contribution measure on all of its items.
        ...
        this.iRevisionDataSet = this.iRevisionDataSet.map(new MapFunction<IRevision, IRevision>() {
            ...
            public IRevision map(IRevision iRevision) throws Exception {
                IDiffer differ = new EditOnlyDiffer();
                IDifference diff = differ.calculateDiff(iRevision.getParentRevision(), iRevision);
