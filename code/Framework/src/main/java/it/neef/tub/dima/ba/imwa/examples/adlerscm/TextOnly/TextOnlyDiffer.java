...
/**
 * Differ for Adler's TextOnly contribution measure, by implementing the txt(v_i, v_j) text contribution.
 *
 * @see <a href="https://users.soe.ucsc.edu/~luca/papers/08/wikisym08-users.pdf">the paper</a>
 * Created by gehaxelt on 03.01.17.
 */
public class TextOnlyDiffer implements IDiffer<IDifference<Double>, IRevision> {
    ...
    public IDifference<Double> calculateDiff(IRevision current, IRevision next) {
        ...
        if (current == next) {
            ...
            ld.setDifference(new Double(newInCurrent.size()));
            return ld;
        }
        ...
        ld.setDifference(new Double(stillPresentCount));
