 ...
 * SkipXMLContentHandler and thus skipping consecutive revisions by the same author as described
 * by Adler et al.
 ...
 */
public class SkipDumpParser implements IDumpParser<IPage> {
    ...
    public void parseDumpData() throws Exception {
        ...
        pageDataSet = pageSplit.map(new MapFunction<Tuple2<LongWritable, Text>, IPage>() {
            ...
            public IPage map(Tuple2<LongWritable, Text> longWritableTextTuple2) throws Exception {
                ...
                // The SkipXMLContentHandler should also take care of skipping consecutive revisions by the same author.
                ArrayList<IPage<IRevision, IPageview>> pages = SkipXMLContentHandler.parseXMLString(longWritableTextTuple2.f1.toString(), this.dumpParserConfig);
